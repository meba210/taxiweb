...
type Route = {
  id: number;
  // Use station_name instead of StartTerminal
  station_name?: string;
  EndTerminal: string;
  Distance?: number;
  Duration?: number;
...
export default function RouteDetail() {
    ...
    try {
      ...
      if (routeData) {
        ...
        const mappedData: Route = {
          //id: routeData.id,
          // Use station_name if available, otherwise use StartTerminal or default
          station_name: routeData.station_name || routeData.StartTerminal || "Main Station",
  ...
  const statusInfo = getStatusInfo(route.Status || 'active');
  const stations = route.Stations || [route.station_name || 'Main Station', route.EndTerminal];
  
  // Get start terminal display name
  const startTerminal = route.station_name || route.StartTerminal || 'Main Station';
          ...
          <Card className="shadow-sm">
            ...
            >
              <Descriptions.Item label="Start Terminal (Station)">
                <div className="flex items-center">
                  <CiLocationArrow1 className="mr-2 text-green-500" />
                  <span className="font-bold text-green-600">{startTerminal}</span>
                </div>
              </Descriptions.Item>
                ...
                {stations.map((station, index) => (
                  ...
                  >
                    <Card size="small" className="w-full max-w-xs">
                      <div className="font-medium">{station}</div>
                      <div className="text-xs text-gray-500">
                        {index === 0 ? 'Start Station' : 
                         index === stations.length - 1 ? 'End Terminal' : 
                         'Intermediate Station'}
                      </div>
                    </Card>
                  </Timeline.Item>
