...
const EditRoutesModal: React.FC<EditRoutesModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && route) {
      // Use station_name or StartTerminal if available
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.put(`http://localhost:5000/routes/${route?.id}`, {
        // Only send what your backend expects
        EndTerminal: EndTerminal.trim(),
      },
      {
        headers: { Authorization: `Bearer ${token}` },
    ...
    >
      ...
      >
        ...
        <Card
          ...
          bodyStyle={{ padding: '16px' }}
        >
          {/* Start Terminal - Read Only */}
          <Form.Item
            label={
              <Space size={4}>
                <FaMapMarkerAlt size={12} />
                <Text strong>Start Terminal</Text>
                <Tag color="blue" style={{ fontSize: '10px' }}>Read Only</Tag>
              </Space>
            }
          >
            <Input
        ...
        <Alert
          ...
          description={
            <Space direction="vertical" size={2} style={{ width: '100%' }}>
              <Text>• End terminal is required</Text>
              <Text>• Terminal must be at least 2 characters long</Text>
              <Text>• Use clear, descriptive terminal name</Text>
            </Space>
          }
          type="info"
          showIcon
        ...
        <div style={{ 
          ...
          <Text type="secondary" style={{ fontSize: '12px' }}>
            <FaExclamationCircle style={{ marginRight: '4px' }} />
            Route ID: {route?.id} • Only end terminal can be modified
          </Text>
        </div>
      </Form>
    </Modal>
